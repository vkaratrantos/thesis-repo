...
int main(int argc, char * argv[])
{
  ...
  auto try_incremental_search = [&](double target_x, double target_y, double target_z) -> bool {
      ...
      // Εργαλεία για τον Απεγκλωβισμό (ΑΥΞΗΜΕΝΗ ΕΛΕΥΘΕΡΙΑ)
      std::uniform_real_distribution<> escape_xy_dist(-0.25, 0.25); // Από 15εκ σε 25εκ.
      std::uniform_real_distribution<> escape_z_dist(-0.10, 0.25);  // Μπορεί να κατέβει και λίγο για να "ξεπιαστεί"

      std::cout << "\n    [ΕΝΑΡΞΗ] Αυξητική Αναζήτηση με Βελτιωμένο Auto-Escape..." << std::endl;
      ...
      while (step_count < max_total_steps) {
          ...
          // 2. Αναζήτηση ενδιάμεσου σημείου
          bool found_intermediate = false;
          int max_attempts = 20; // ΑΛΛΑΓΗ: Αυξήθηκε από 10 σε 20
          ...
          if (!found_intermediate) {
              std::cout << "    [!] Ενεργοποίηση Διαδικασίας Απεγκλωβισμού..." << std::endl;
              
              bool escaped = false;
              int escape_attempts = 100; // ΑΛΛΑΓΗ: Αυξήθηκε από 30 σε 100 για σίγουρη διαφυγή
              ...
              for (int e = 0; e < escape_attempts; ++e) { 
                  ...
                  // Βεβαιωνόμαστε ότι δεν χτυπάει πάτωμα μετά την κάθοδο
                  if (escape_candidate.position.z < 0.05) continue; 
              ...
              if (!escaped) {
                  std::cout << "    [ΣΦΑΛΜΑ] Το ρομπότ εγκλωβίστηκε ολοκληρωτικά παρά τον εκτεταμένο έλεγχο." << std::endl;
                  return false;
              }
          }
  ...
  std::cout << ">>> Σύστημα Έτοιμο: Ενεργός Αλγόριθμος Incremental Greedy Search (React: Balanced)." << std::endl;
