...
// Gripper closing/opening speed. MoveGroupInterface defaults to 0.1 and nothing
// ever overrode it, so these halve that -- about half the previous speed.
// Adjust both together; the travel is short enough that acceleration dominates.
static constexpr double GRIPPER_VEL_SCALE = 0.05;
static constexpr double GRIPPER_ACC_SCALE = 0.05;
...
static constexpr double GRASP_Z        = 0.12;      // TCP height at the tube
static constexpr double POUR_Z         = 0.20;      // TCP height above the mixer
static constexpr double APPROACH_DIST  = 0.1;      // vertical descent to grasp
...
int main(int argc, char * argv[])
{
    ...
    // The gripper never had any scaling set, so it planned at MoveGroupInterface's
    // default of 0.1. Half that closes the fingers at roughly half the speed.
    //
    // This only does anything because the drivers were made to honour
    // time_from_start: fake_robot.py replayed a flat 20 ms per waypoint, and
    // moveit_bridge_7.py's gripper_callback jumped straight to the last point
    // ("Gripper Snap Command"). Both discarded the timing, so scaling here was
    // inert. Fixing the driver is what makes this knob real.
    gripper_interface.setMaxVelocityScalingFactor(GRIPPER_VEL_SCALE);
    gripper_interface.setMaxAccelerationScalingFactor(GRIPPER_ACC_SCALE);
